...
use crate::{G_WINDOW, ui::btn_action_row::{on_open_dir_clicked, on_save_changes_clicked}};
...
pub fn listen_for_events(tx: Sender<CommEvents>) {
    ...
    accel_group.connect_accel_group(accel_key, accel_mods, gtk::AccelFlags::VISIBLE, move |_, _,_ ,_| {
        on_open_dir_clicked(&tx.clone());
        true
    });

    G_WINDOW.with(|w| {
        w.borrow().clone().unwrap()
